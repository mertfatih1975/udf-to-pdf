...
def guclu_parser(data):
    try:
        ...
        # STRATEJİ 2: Zlib imzasını (78 9C) tüm dosyada ara
        ...
        return ["HATA: UDF icerigi bulunamadi. Dosya sifreli olabilir."]
    except Exception as ex:
        return [f"TEKNIK HATA: {str(ex)}"]

def parse_xml_to_lines(xml_content):
    root = ET.fromstring(xml_content)
    lines = [elem.text.strip() for elem in root.iter() if elem.text and elem.text.strip()]
    return lines if lines else ["Belge metni bos."]

# --- PROFESYONEL ARAYÜZ (ONAY SİSTEMLİ) ---
HTML_UI = """
<!DOCTYPE html>
<html lang="tr">
<head>
    ...
    <style>
        ...
        button { border: none; padding: 15px; border-radius: 10px; cursor: pointer; font-weight: bold; color: white; transition: 0.3s; opacity: 0.5; pointer-events: none; }
        ...
        .kvkk-check { margin: 20px 0; font-size: 12px; color: #cbd5e1; display: flex; align-items: center; justify-content: center; gap: 8px; cursor: pointer; }
        .kvkk-link { color: #38bdf8; text-decoration: underline; }
    </style>
</head>
<body>
    <div class="box">
        <h2 style="color:#38bdf8; margin:0 0 10px 0;">UDF PRO <span style="color:white">ELITE</span></h2>
        <div class="security-badge">🔒 <b>Güvenlik:</b> Dosyalar sunucuda depolanmaz, RAM üzerinden anlık işlenir ve kalıcı olarak silinir.</div>
        ...
        <form id="uForm" method="POST" action="/" enctype="multipart/form-data">
            ...
            <label class="kvkk-check">
                ...
                <span><a href="#" class="kvkk-link" onclick="alert('KVKK: Verileriniz 6698 sayılı kanun uyarınca sadece anlık işleme amacıyla kullanılır ve asla kaydedilmez.')">KVKK Aydınlatma Metnini</a> okudum ve onaylıyorum.</span>
            </label>
            ...
            <div class="btn-group">
                ...
                <button type="submit" name="mod" value="word" id="btnWord" class="word" onclick="run()">WORD (DOC)</button>
                <button type="submit" name="mod" value="txt" id="btnTxt" class="txt" onclick="run()">HIZLI TEXT (TXT)</button>
            </div>
        </form>
        <p style="font-size:10px; color:#475569; margin-top:25px">© 2026 FATİH MERT | Ofis Gökçadır - BURSA</p>
    </div>
